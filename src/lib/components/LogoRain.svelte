<script lang="ts">
	...
	$effect(() => {
		...
		const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 359.97 301.96"><path fill="${LIME}" d="${PATH}"/></svg>`;
		const img = new Image();
		let imgReady = false;
		img.onload = () => (imgReady = true);
		img.src = 'data:image/svg+xml;utf8,' + encodeURIComponent(svg);

		const GW = 32; // ancho del logito (~ tamaño del logo de la navbar)
		const GH = GW * (301.96 / 359.97);
		const GRAV = 0.3;
		const BUCKET = 14;
		const MAX = 260;
		...
		let W = 0,
			H = 0;
		let buckets: number[] = [];
		let falling: { x: number; y: number; vy: number }[] = [];
		...
		function setup() {
			...
			buckets = new Array(Math.max(1, Math.ceil(W / BUCKET))).fill(H);
			pile = document.createElement('canvas');
			pile.width = cv.width;
			pile.height = cv.height;
		...
		function floorAt(x: number) {
			const b0 = Math.max(0, Math.floor(x / BUCKET));
			const b1 = Math.min(buckets.length - 1, Math.floor((x + GW) / BUCKET));
			let top = H;
			for (let b = b0; b <= b1; b++) top = Math.min(top, buckets[b]);
			return top;
		}
		function raise(x: number, topY: number) {
			const b0 = Math.max(0, Math.floor(x / BUCKET));
			const b1 = Math.min(buckets.length - 1, Math.floor((x + GW) / BUCKET));
			for (let b = b0; b <= b1; b++) buckets[b] = Math.min(buckets[b], topY);
		}
		...
		function frame() {
			...
			tick++;
			if (tick % 7 === 0 && count < MAX) {
				falling.push({ x: Math.random() * (W - GW), y: -GH, vy: 1 + Math.random() * 1.5 });
				count++;
			}
			...
			for (let i = falling.length - 1; i >= 0; i--) {
				...
				if (f.y + GH >= floor) {
					const topY = floor - GH;
					pileCtx.drawImage(img, f.x, topY, GW, GH);
					raise(f.x, topY);
					falling.splice(i, 1);
				}
			}
			...
			for (const f of falling) ctx.drawImage(img, f.x, f.y, GW, GH);
		}
